...
Class Mongo_db{
	...
	* --------------------------------------------------------------------------------
	* 
	* Update the Mongo DB with the parameters and KEY PROVIDED in a table and return the updated value
	*/

	public function updateCustom($table,$key,$param)
    {
        $collection = $this->db->selectCollection($table);
        return $collection->update(array('_id' => new MongoId($key)), array('$set' => $param),array("upsert" => false));   
    }
	/**
	* --------------------------------------------------------------------------------
	* //! Insert
	...
	public function update($collection = "", $options = array())
	{
		...
		catch (MongoCursorException $e)
		{
			if(isset($this->debug) == TRUE && $this->debug == TRUE)
			{
                ...
                error_log("Update of data into MongoDB failed: {$e->getMessage()}");
			}
			else
			{
...
//                error_log("Update of data into MongoDB failed". {$e->getMessage()}");
	...
	* --------------------------------------------------------------------------------
	*
	* Delete documents from the passed collection based upon certain criteria
	* @author: Ali Zain
	* @usage : $this->mongo_db->deleteEntry('foo', ID of the entry);
	*/
    public function deleteEntry($collection,$id)
    {
        if (empty($collection))
		{
			show_error("No Mongo collection selected to delete from", 500);
		}
			$this->wheres['_id'] = new MongoId($id);
        try
		{
 ...
 /**
	...
	* Delete documents from the passed collection based upon certain criteria
	* @author: Ali Zain
	* @usage : $this->mongo_db->deleteEntry('foo', $param=array());
	*/
    public function deleteEntrywithParam($collection,$param)
    {
      // print_r($param);
